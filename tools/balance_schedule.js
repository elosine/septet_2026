...
// of that technique's range — at velocity 127 and 64. One note at a time, a fixed timetable,
// so the recording can be sliced by the same file:
//
//   node tools/balance_schedule.js [--note 1500] [--gap 1000] [--lead 3000] [--vels 127,64]
//                                  [--only violin1,cello] [--out probes/balance_schedule.json]
//
...
for (const inst of ORDER) {
    ...
    for (const vel of vels) for (const pitch of pitches) {
        notes.push({ i: i++, inst, label: I.label, tech: tech.key, techLabel: tech.label, port: tech.port || I.port, ch: tech.channel || 1,
...
const schedule = { generatedAt: new Date().toISOString(), source: 'sandbox/instruments.js', order: ORDER.filter(k => !only.length || only.includes(k)),
                   leadInMs: leadMs, preMs, noteMs, gapMs, instGapMs, vels, fracs: FRACS, totalMs: t, notes };
...
for (const inst of schedule.order) {
    const n = notes.filter(q => q.inst === inst); if (!n.length) continue;
    console.log('  ' + n[0].label.padEnd(9) + ' ' + n[0].port.padEnd(7) + ' ch' + String(n[0].ch).padEnd(3) + (n[0].cc0 != null ? 'cc0=' + n[0].cc0 : n[0].ks != null ? 'ks=' + n[0].ks : '      ').padEnd(8) +
