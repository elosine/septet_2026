...
// ---- (A) the commands on a copy ----
{
  ...
  ok(f && f.of === 99 && f.stuck === 0, 'foldFlute over section 3: 99 flute rams, none stuck (got ' + (f && f.of) + ', stuck ' + (f && f.stuck) + ')');
...
if (process.argv.includes('--save')) {
  ...
  ok(rams.length === 99 && !out.length, 'THE SAVE: 99 section-3 rams inside C3–D4 (' + out.length + ' outside)');
}
