...
| **THE PITCH FIGURE — D45 (2026-09-14, §472; the mock §471 `docs/images/morph_header_arrow_cents_mock_2026-09-14.svg`):** the altered head is written to the **nearest EIGHTH tone** — the natural / quarter-tone sign with an **arrow** for ±25 c (the arrow on the nearest sign, Gould) — **and the exact deviation in cents** as a small number above it (Haas / Tenney: the deviation from the written pitch, "+25" · "−14"; a 0 is dropped). Both, always. The tuba's "at least a quarter tone" is superseded. M1: C♮↑ +25 · C♮↓ −25 (D, A likewise; the bass clarinet in written pitch). **All pieces from here; piece #4's rehearsal and performance scores take it when made** (its presentation score stays) | (build, PLAN 2h.2: `accidentals.natural.arrowup/down` from Emmentaler per GLYPH_SIZING; `acc` + `cents` on the header overlay; a text item in the header chain) | **D45** · §469–§472 · CN-80 |
...
| **The pitch figure's general rule — his amendment pending (§473):** never a down arrow — the head at or below the target on the quarter-tone grid, an up arrow, a plus number 0–49 c (D −5 c → D¼♭ ↑ +45; Vn2's D −14 → D¼♭ ↑ +36) — or the signed rule as D45 first recorded (nearest eighth tone, "D −14"). M1 is C♮↑ +25 either way | — | open, his a/b |
| **Held, his to decide at the build (§464):** the drawn floor at 1 for the morph crescendo (now on an ABSOLUTE curve, D46; the gliss is pitch, no floor) · the piano's 44 notes in the morph section — the section-1 strike device with a plucked mark, or another · the beating indication — parts only, or on the score | — | open |
