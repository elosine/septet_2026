...
| 10 | 2026-09-12 | chord-per-onset card, `max` | set `max 4` expecting four of the six non-piano players | `max` counts the piano in — *"max seems to take into account the piano ... I would have preferred if the piano is outside the count. So I'm dealing with the other six. so I'd know that if I select four, the two remaining non piano instruments are playing the next one, etcetera. But just leave it for now."* | `max` = the ensemble six only; the piano set by `piano count` / the piano row alone | N — his "leave it for now, I just want to start"; the workaround is to count the piano into `max` by hand |
