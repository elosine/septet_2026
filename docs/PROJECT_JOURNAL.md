...
| **N3b** | **► sitting D** — steps 5–6 the groups and `--pairs` (2g.2 · 2g.4 · 2g.5, D51): cut the piano run 581.21–624.00 into pairs (gaps < 0.4) and fours (< 0.25, from 607.27) · the six other parts' 46 pairs + 2 triples · his eye on the crowding (the build's geometry check already flags 623.55) · **sitting E** — steps 7–8 the page rule (first: align `dynamicBands` to the eight-step scale, §520) + the surge device (CN-83 · CN-84) · **sitting F** — steps 9–10 re-render, demo video | Opus (Fable for 8's design) | **yes — wrap on Opus: `/checkpoint` · `/clear` · `/postclear`** |
