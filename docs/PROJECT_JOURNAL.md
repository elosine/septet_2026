...
| **N4** | **DONE 2026-09-13 (§455–§461): the beamed-group standard decided; the pair at 43.3 built; SECTION 1 COMPLETE under it (its only gap under 0.4 s); the trill conflicts closed.** ► NEXT: his eye on the 43.3 pair (accents beam-side or head-side) → the piano's ONE 169-note run after 183 s: how a long accelerating run is beamed — the design conversation (§461) → a `--pairs`-shaped build flag for the 46 pairs + 2 triples in the six other parts → the go-to-time + HOME in the presentation score (§456 item 5) | **Fable** for the run's design · **Opus** for the pairs flag and the go-to-time | at the mode change |
