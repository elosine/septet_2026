...
**NOW ✦** 2026-09-16 late (session 14, Fable; checkpoint on Opus) — **N1 THE PERFORMANCE INSTRUCTIONS, PART-WAY** (RUNNING_LOG §565–§581): the draft page `docs/notation_instructions/index.html` (the tuba's page, nouns changed, served at :5300/docs/notation_instructions/) — he is revising it in chat, section by section; **the prose is his** (§569). Done: the septet's conduction, crescendo and trill images (`tools/capture_lane.js`, ported) · the Gradient Curves section to his dictation · his beating paragraph · the morph sequence chart, BLOOM + SPECTRAL (`tools/gen_morph_chart.js`). Open: the Trills text · the beating section's tuba sentences, demo list and notation image · Multiple Tempos · the NITS items. **► NOW: resume his dictation pass (journal §2 CHECKPOINT has the list). Opus for dictation, Fable for design.** Then the print score (2b). Sections 1–3 notated, the video and render approved (session 13).
