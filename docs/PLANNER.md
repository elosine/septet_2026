...
spec written; step 0's formula file rejected on hearing; the model moves to piece #2's human-sampled timing — option A built: his three trill sample files → `bank/trill_timing_db.json`; the curve test `scores/trill-curve-test.json` — now a zone with the trill embedded as a MIDI snippet, played by piece #2's timestamped zone tick (RUNNING_LOG §102; the plain-note path is frame-polled: NITS) — awaits his ears; his captures as `midi/*.mid` for auditioning articulations; phase 1 on his go); the 0d bridge sweep of the strike techniques' ranges and lengths
