...
**NOW ✦** 2026-09-10 — **A DAY OF DEFECTS, AND THEY WERE ALL ONE DEFECT** (RUNNING_LOG §340–351). Nine faults reached him in one session and **every one was the same shape: a rule, a number or a behaviour wired into ONE code path and not its sibling.** The run needed players the pick never supplied (§342) · *a piano cannot swell* enforced in the chord deal only (§343) · **two Insert buttons writing different KINDS of object** (§345) · two CC7 laws, live and export (§346) · a strip margin hard-coded against a panel that measures wider (§347) · **the chord feature returning early in the mode named after it** (§348) · the drawer's Hear and the score's playback differing in THREE ways at once — range, curve and velocity (§349) · a secco cut overwritten 5 ms later by its own ramp (§350). All were found by READING; none was a reasoning failure, which is the answer to his twice-asked question about a stronger model. **His rule, now the through-line of PLAN 1q:** *"insert at playhead means insert what I'm listening to at the playhead"* — the audition and the write must be ONE implementation, not two kept in agreement by hand. **What is fixed and unheard:** keep rhythm + orchestrate on a new pick · the piano out of crescendo passes · Insert writing Hear's own numbers (0 mismatches measured) · secco with a neighbour guard · the card's MIDI numbers and a held accent. **What is COLLECTED and unbuilt:** the rhythm-drawer remodel (§AE) · notes outside the harmony and a pinned player (§AF) · the two articulation sets, percussive and all-spiccato (§AB1-c). **The one thing he must be told:** the chord tool he specified lives in **NOTES mode**, not chords mode.
