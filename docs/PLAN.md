...
- **1t — The chain: strikes alternating with crescendos, ITEM BY ITEM in the score — the crescendo panel on a selection · the playhead-to-end key · a `unison` rhythm · free/busy ticks · the piano block on the plain strike (CN-68 · CN-69 · CN-70 · CN-71 · CN-72)** — `built 2026-09-12 on Opus (RUNNING_LOG §419; STRIKES_TOOL §AI “how to use it”) — all six steps; `check_cresc_panel.js` 30 green on the first run, check_fill/containers/cresc_deck + 86 + test_identity + the tuba battery green; his eight steps of §416 walked with real clicks on `zz-ai-1t-walk` at :5301. **In his hands all evening 2026-09-12 (RUNNING_LOG §420–§426): six fixes, three rule changes (D39); the draft tagged `Scattered_Substance-finalDraft_1.0`.**` — his ask (CN-68): *"create a series of strikes, just single note ones that alternate with crescendos … I need to figure out a way to treat the piano so they can only be on the strikes"*; the shape he settled on (CN-69–71): **the drawer makes only the strikes; every crescendo is made in the SCORE from the strike that launches it, by selection — nothing added to every note.** *Why:* the chain is his next passage, and the drawer's chords and fill modes are deprecated at his word (§413), so the crescendos come from the score, one selection at a time, with the placements as clicks rather than numbers.
...
- **2h — Section 2 notated: the morphs** — `planned 2026-09-14 (RUNNING_LOG §464–§467; NOTATION_STANDARDS §3; D44)` — **Result when done:**
  ...
  2. **The tool's septet fixes, with checks** — `built 2026-09-14 (RUNNING_LOG §477): tools/test_morph_notation.js 162 pass, proven red; the tuba frozen and unchanged; the breath-peak arc's tangents limited (plain Catmull-Rom sagged 8.4 %) — for his eye` — `morph_overlays.js` / `notate_morph.js`: parts = layers under `tracks.length` (not
     ...
     `Core.evalWaveCurve` 0–1, × the `cc7Fade` weight — D32, M1's 17 s fade from silence is in CC7, not the level), Catmull-Rom through them,
     no min–max normalisation, sampled at 100/s (≥ 101); NO floor on the morph (D47 — the trills' `curveFloor` stays theirs); the header's
     marks FIXED niente · arrow · `fff` (D46; `endMark: 'fff'` stays)** · the 20 c threshold with the alert (D44) · **the pitch figure per D45 as settled (§474): the destination head spelled to the
  ...
  3. **M1 folded into the MAIN file** — `done 2026-09-14 (RUNNING_LOG §478–§479): his eye — "ok m1 good"` — `--morph grp-morph-01` added to the recorded build (D41), rebuilt; headless: six headers,
  ...
  7. **The practice videos with the beat rates (D48)** — `done 2026-09-17` *(his "videos good"; online, linked from the instructions page; archived — RUNNING_LOG §602)* *(was `later, the performance stage`;
     ...
     **THE BUILD PLAN: `docs/plans/BLOOM_PRACTICE_VIDEOS.md`** (RUNNING_LOG §595–§597) — the tuba's final form, located and read
     ...
     - **2h.7.1** Prove the two new uses — a two-lane export on the septet frame (non-adjacent lanes) · a pair-only bridge render — `done 2026-09-17` (§599)
...
- **2i — Section 3 notated: the strikes, the groups, the crescendos, the dynamics** — `planned 2026-09-14 (RUNNING_LOG §505–§511; the top line his: "top line good")` — **Result when done:**
  ...
  5. **The piano's run from 581 s cut into groups; the fours at the end** (= 2g.2 · 2g.4) — `BUILT 2026-09-14 (RUNNING_LOG §522): --groups 444-624.1 —
     39 pairs · a triple at 606.27 · 22 fours from 607.02 (cut from the end, landing on the 620.32 four); 41 groups cross the staves; 55 crowding flags in the run; his eye owed · RE-CUT 2026-09-14 by his times (§526–§528): 16 singles to 586.96 · 41 pairs from 587.32 · triple 611.05 · 17 fours from 611.72 (--groupCuts 2@587.32,611.72) · --max16 · a beam-side ottava clears its beam with the group's accents below the notes · a member's chain clears its group accent (§527)` — his eye on the crowding.
  6. **`--pairs` for the six other parts** (= 2g.5) — `BUILT 2026-09-14 (§522), by the same --groups rule: 46 pairs + D51's two triples; no group dynamics (step 7's); his eye owed · after his note moves (§523–§524): Fl 2 pairs · BCl 7 · Vn1 5 + a triple 620.56 · Vn2 8 · Va 4 + triples 622.01, 623.20 · Vc 4 + 488.51` — 46 pairs · the two triples as groups of three (D51: Vc 488.51 · Va 622.01).
  7. **The page rule for section 3's dynamics** (CN-83) — `BUILT 2026-09-14 (§529): dynamicBands = the eight steps (only section 3's marks change) · --dynOnChange 444-624.1 · 42 marks, six per part p→fff, each on the part's first strike in the band, group members included · "changes" against the last written dynamic (ready for 8's surge) · his eye owed` — one mark per part where its band changes, on that part's first
  ...
  9. **The audio re-render** (RENDER.md) — `RENDERED 2026-09-16 (Opus, RUNNING_LOG §553–§554) · his ear owed` — the save of 16:03 captured (0 writes), exported with every check green (the bend check (e) fixed to read each instrument's measured range: 183 bends in place), rendered through the bridge (the guard's heartbeat race fixed) — 630.100 s · float +1.9 dBTP → −2.9 dB → −1.0 dBTP · −21.8 LUFS · LRA 13.8 · first sound 3.7 ms · `notation/audio/piece-septet.wav` served. **His ear:** the sync · 2j.3 the piano's section-2 dynamics · 2j.7 the morph notes' starting pitch · 2j.8 the surge run in tune.
  10. **The demo video** — `DONE 2026-09-16 (RUNNING_LOG §556–§564) — archived as the submission copy · online: https://youtu.be/x8EZ3B1EvbE` (`tools/export_video.js`; PLAN 2b's sync proof as #4's PHASE 5; the cover title *Scattered Substance* tentative, CN-64 — his word first). **His shape (2026-09-16, RUNNING_LOG §556–§557):** the wide shot with close-ups, *"a new random selection for the close up sections and incorporating the transitions we used in the tuba video"*. **The top line (his "good run thru as much as you can independantly"):**
      1. **The frame** — the wide shot draws the septet: seven parts, the piano's grand staff, part labels, no tuba lanes. `BUILT 2026-09-16 (RUNNING_LOG §558): the exporter mirrors renderContainerView (ensemble parts, weights, staves, the §404 buffer, labels); pages 1 and 50 compared with his page in the running app — six systems and the furniture identical, the BCl differing by design (C, bass clef)`
      2. **The close-ups** — the top and bottom halves at 2×, fitted to the septet's frame. `BUILT 2026-09-16 (§558): 2× does not fit (y = 1080 falls inside Vn1; the strings are 1122 px) → groups (Fl · BCl · Pno / the strings) at 1.85×, centred, the rest paper (registry realizations.video-cut); proven on 16 moments — the partition row empty, nothing sliced`
      3. **The cut** — the septet's sections in the cut tool, a few fresh seeds drawn against them, his pick. `scanned 2026-09-16 (§559): 300 seeds; 53 and 72 give two close-ups in every section, 4/4 — HIS PICK: seed 53 (§561, "a, seed 53") → notation/video/cut-list.json`
      4. **The transitions** — the 5-frame cross-dissolve at each cut, checked on septet frames. `checked 2026-09-16 (§560): two 5.5 s clips at seed 53's cuts, the blend over frames 81–85 measured · his eye: "clips look good" (§561)`
      5. **The render** — wide, close-ups and cut, laid under the new audio render. `RENDERED 2026-09-16 (§562): notation/video/renders/piece-septet-V-CUT-seed53.mp4 — 18 903 frames = 630.100 s, audio 630.100 s, 14.8 min; the picture holds past 625 s so the WAV's tail rings`
      6. **The proof** — the sync check (the tuba's PHASE 5), then his eye on the finished video. `DONE 2026-09-16 (§562–§563): duration equal · A/V start 0 · cut sources 0.46–2.07 % vs 9–15 % wrong-source controls (+ §558's page identity) · his eye "video good" → notation/video/approved/2026-09-16-submission/`
...
- **2j — The proofing pass, all parts** — `FIXED 2026-09-16 (Fable, RUNNING_LOG §537–§550; his: "I'm proofing the notation in all the parts … collect the notes I'll give … fix them all at the end of the proofing") — 2j.1–2j.5 in the save (14 techniques, 29 velocities) and on the page (the word at the top · 42 pairs, 17 fours, the last note alone · every three 2 + 1), 2j.7–2j.8 in the playback (the bend pre-armed and centred at exit; export check (e)) — heard at the re-render; MAIN rebuilt (--groupCuts 2@587.32,611.5 --threes 2+1), every battery green incl. piece #4's, the page proven · HIS EYE owed on each spot (F5; Reload his composer tab)` —
  ...
  **The notes — verbatim, dated, in the order given; the AI's reading marked and kept separate:**
  ...
  2. **2026-09-16 · "sempre secco", all six parts** — *"lets move sempre secco text to the top for all parts, so it will be at the tip of the crescendo end and lets add some horiz space between the right edge of curve and left edge of text (the s of sempre), just the small gap worth"* — *(AI's reading, not fixed:)* the word leaves the below-chain and sits at the CUT EDGE's top — the curve's sharp top-right corner (full lane height at the note end, render.js) — its left edge (the "s") the small gap right of the cut edge's stroke; the same for every part's first swell. **Settled by him (2026-09-16, §540):** *"the standard small gap we are using, the gap between notehead and staccato dot; text top justified with the top of the crescendo"* — the gap = **0.15 ss** (`nhDotGapSs`, the staccato-dot gap; the same 0.15 as `dynStemGapSs`); the text's **top edge on the curve's top** at the cut edge (the em box hanging below the tip), left edge at the cut edge + 0.15. Where: `layout.js`, the block after the instruction slot (§531's `seccoShown` text) — off `placeChain`, a fixed y at the lane top + the gap in x; `test_surge_run.js` re-pinned (the six texts: t = cut edge, dx = the gap, y at the top); NOTATION_STANDARDS §2's secco row updated.
  3. **2026-09-16 · piano · section 2 (the morph sections)** — *"in the piano parts in section 2 seems some of the dynamics aren't picked up in the midi, This isn't worth too much effort and troubleshooting, but let's see if we can't have the notated dynamics reflected in the final audio render."* — *(AI, measured, not fixed:)* the piano's 44 section-2 notes (205.85–427.75; 32 plucked, 12 ordinary) are all captured-note (plain) mode, so they sound at their `recVel` on BOTH the page's MIDI and the render — what he heard is what the render plays. Their written marks are D50's, derived from the ensemble's level, not from the velocity; **29 of the 44 play a band other than their mark** — among them 278.34 plays p (31) under a written ff · 318.48 f (90) under pp · 427.75 mf (78 · 76) under ppp · the 345–402 plucks ff (104–118) under f. The fix, minimal: the 29 take their mark's step value (`recVel` ← the eight-step scale: ppp 9 · pp 18 · p 37 · mp 55 · mf 72 · f 90 · ff 109 · fff 127 — the field `stepDynamics()` writes, §520), the 15 already in band untouched; Save → R — the marks do not move (D50 reads the ensemble, not the velocity); the re-render then carries the sound. A script over the save through the note card's field, one undo step's worth; his ear on the loudest changes (278.34 ↑, 318.48 ↓, 427.75 ↓) after.
  4. **2026-09-16 · piano · 611.1, and the triples in general** — *"give me some alternatives to 2 beamed then 3beamed then 4 beamed; and in general to the 3 beamed groups; This risks to sound a bit like compound meter … how to deal with an odd number without sounding like compound meter … Also, we can rebeam all of the groups of four and just leave the last note at the end its own GC … the third partial of the three group would start a new four and just move on down to the end. And then the last partial of the piece in the piano would just be its own GC. That may be the best solution."* (CN-87, RUNNING_LOG §545) — *(AI, measured:)* the run today: 16 singles · 41 pairs · the triple 611.05 · 611.28 · 611.50 · 17 fours from 611.72 to 624.00. **His (c) in numbers: 42 pairs (587.32 → 611.28) · 17 fours from 611.50 (69 notes, every gap 0.15–0.22 s) · the last note 624.00 its own GC.** Duple throughout; the run's last impulse an arrival. **Recommended — and DECIDED by him 2026-09-16: "pno at end my solution, 2s then 4s then last one single."** The alternatives (§545): (B) the odd note as a single at the START of the fours (a pickup, weaker); (C) compose it away — one note more after 624.00 → …4 · 4 · 2, or one fewer → the fours end exactly. **The other triples:** Vn1 620.56 isolated — leave (a lone three is a figure, not a meter) — **his verdict 2026-09-16: "vn1 at 620 leave it", then corrected: "sorry vn1 at 620 2+1"** → 620.56 · 620.93 a pair, 621.30 its own GC · Vc 488.51 after a pair (2-3, borderline) — **his verdict 2026-09-16: "vc at 488 2+2+1"** → the pair 487.76 stays · 488.51 + 488.86 a pair · 489.18 its own GC · Va 622.01 + 623.20 back to back (3-3, the clearest risk; 0.5 s apart, so no 2+4 merge — each as 2+1, or a note moved, or left) — **his verdict 2026-09-16: "va at 622 all 2s instead of 4s" → on the 0.5 s gap: "no then 2+1 , 2+1"** → each three becomes a pair and a single GC: 622.01 · 622.36 + 622.70 alone · 623.20 · 623.52 + 623.84 alone. **The fix (tool):** `--groupCuts 2@587.32,611.50` with the fours counted from the cut FORWARD and the remainder single at the END (a small addition to `notate_section`; today the fours are cut from the end) · `test_cross_staff` re-pinned (41·1·17 → 42·0·17 + the 624.00 single; 153 grouped → 152 + 1).
  5. **2026-09-16 · the six parts · every strike's percussive articulation (a RULE)** — *"vln1 36.28 missing bartok pizz; 49.67 same, actually for all the 6 instruments -piano they should all have the same percussive articulations except for morph and surge sections"* — *(AI, measured on MAIN, not fixed:)* of the 999 strikes in the six parts outside the morph (205–428) and surge (526–560) windows, **14 are not percussive** — 13 strings on `senza_vel` and the flute's `ord` (note 1), all in section 1: Va 36.13 · Vn1 36.25 · Va 43.10 · Vn2 43.33 · Vn2 49.52 · Va 49.63 · Vn1 49.68 · Va 58.87 · Fl 59.56 · Vn1 114.44 · Vn1 119.67 · Va 120.98 · Vc 123.34 · Vn2 167.90 (ids wc-899 · 900 · 825 · 828 · 851 · 853 · 854 · 869 · 877 · 1034 · 1058 · 1066 · 1083 · 1278). Section 3's strikes are all percussive already. The fix: each takes its part's own articulation — Vn1/Vn2 `bartok_vel` (Bartók pizz) · Va/Vc `gettato_vel` (jeté — **his, 2026-09-16: "va vc always jete"**) · Fl `pizzicato` (tongue ram, folded — note 1) — through the note card's technique path so the captured note is re-taken; Save → R; the re-render carries the sound. **The rule for the standards (his, + "and trills are exception as well", then the precision: "in the trills section, there are one shots very close or between trills … The actual trills themselves, not percussive, just the ordinaro or senza vibrato. But the ones in between that have the GCs, those get the percussive treatment"):** outside the morph sections and the surge run, **every strike (a GC one-shot) in the six parts is its part's percussive articulation, wherever it falls — inside the trill span included**; the trills themselves (IR env `trill`) are ord / senza vib and are not strikes. The guard: 0 sustained-family events with env `strike` in the six parts outside the morph and surge windows — the trills excluded by their env, not by a time window. The 14 stand (four of them, 114.44–123.34, sit inside the trill span and are exactly his "ones in between").
  6. **2026-09-16 · violin · the morph section's dynamics vs the curve** — *"in the morph section, the violin volume slash dynamics doesn't seem to correspond to the curve. Again, this is only impacts the audio render, but if there's a relatively easy fix to this, we can try it. It might have something to do with the CC7 round robin. I can't remember if we used several tracks for the CC7 or just one, but we might have to, if it's the round robin, we might have to set the next one well before it's routed and used at the next breath."* — *(AI, measured, not fixed:)* **what he heard while soloing is the page's MIDI, which is a known gap for these notes** — the morph section's held notes play there at a flat velocity 100 with the anchor-scale CC7 (§442 item 4), so their loudness cannot follow the curve on the page. **The render's capture does follow it:** Vn1's 27 morph notes never overlap; each takes its own curve channel in rotation (ch 2 → 3 → 4, D11's pool, CN-50), pre-armed 0.15 s before its onset (his "set the next one well before" is already the case); the CC7 stream tracks the level — 215.50 → 221.88: CC7 114 · 125 · 120 · 115 · 108 at the quarter points against the level 0.65 · 0.86 · 0.78 · 0.67 · 0.53. No round-robin fault visible. **What is left to judge is the render itself, by ear** (♪ render, 209–228 s): if its loudness still reads flat, the suspect is the measured CC7 map's compression (levels 0.27–0.92 land on CC 104–127 — most of the map's change is below that) or the fade weight, not the rotation. His ear first; a fix only if the render is wrong. — **CLOSED 2026-09-16, his: "if you confirm that the render midi is correct then no need to test I'll trust the audio render will be ok"** — confirmed (the capture's CC7 follows the curve, each note on its own channel, pre-armed); no fix, no test.
  7. **2026-09-16 · the morph sections · pitch jumps at the note boundaries (Vn1 340.24 "maybe others")** — *"same with pitch, there are jumps in m2 for vln 1 at 340.24 in the midi, maybe others"* — *(AI, measured, not fixed — and this one IS in the render:)* the composed line is continuous — at every one of the 101 morph note boundaries in the six parts the sounding pitch carries over within 0.7 c (Vn1 340.20 → 340.25: A3 + 61.6 c → B♭3 − 38.1 c, the same pitch, re-spelled at the semitone crossing). **The render breaks it:** the new note's channel is pre-armed 0.15 s ahead with CC0 and CC7 but NOT with its bend, so the note starts at its nominal pitch and its first bend arrives 1–16 ms after the note-on (Vn1 340.25: note-on 340.252, bend −38 c at 340.267). **62 boundaries start with a bend over 10 c, none pre-armed** — the worst Vn2 348.86 (−75 c) · 391.38 (−64 c) · Va 328.69 (−60 c) · Vn2 407.82 (+54 c) · 340.96 (−53 c) · Va 346.77 (+50 c). Each is a blip of that size for one tick. The fix: pre-arm the bend with the CC0/CC7 pre-arm — the new channel takes the note's initial bend at −0.15 s (composer.html's prearm block; `midiplayer.js` the same for the page); Save nothing — the capture re-run at the re-render (2i.9) carries it; a check in `export_midi.js` (every morph note-on preceded by its bend on its channel).
  8. **2026-09-16 · the surge run · out of tune in the render — the pitch bend never reset after the morph sections** — *"also the surge section, sounds maybe out of tune or could be my ear, this is the render; lets check and make sure the pitch bend is being reset after the morph section"* — *(AI, measured on the render's capture — his ear was right:)* **none of the 212 morph-section note-offs is followed by a bend-to-centre on its channel** (the composer's playback centres the bend only at flush/stop, never at a bent note's exit); so at 526.0 s **ten curve channels still carry a morph bend** — Va ch2 +84 c · BassCl ch3 +69 c · BassCl ch4 +57 c · Vn1 ch2 +27 c · Va ch3 +12 c · Vn2 ch3 +12 c · Vn1 ch3 +4 c · Vn2 ch4 +2 c · Fluteb ch4/ch6 +1–2 c (the measured range 1.99 st, `BEND_RANGE_CENTS` 199) — and **45 of the 78 swells start on a channel with a leftover bend**: Va 526.79 (+84 c), BassCl 528.00 (+69 c), Vn1 529.18 (+27 c), BassCl 532.56 (+57 c) … The strikes are untouched (ch 1 MAIN carries no bend). The fix: the residue cure at the note's EXIT — a bent note's note-off sends bend-centre on its channel (what the page's `midiplayer.js` already does, line ~105; composer.html's `tickCurvePlayback` only at flush) — plus a check in `export_midi.js`: no note-on on a channel whose bend is off-centre unless the note itself carries a bend. No change to the save; the re-render (2i.9) carries it — this and note 7 are one small change in the same block.

- **2k — The bass clarinet in C: the transposing-instrument mechanism** — `BUILT 2026-09-16 (Fable, RUNNING_LOG §552; decided §541–§542, D55 / M5): the registry override on the presentation realization (video-jury, which print borrows) · Layout.ensembleFor · both exporters now pass the realized ensemble + techniques (they passed neither before — the presentation was laid out as the tuba) · the morph headers made re-spellable (q · dir · writtenAt in the overlay; spellHeads shared) · the C page proven on MAIN (203 heads within two ledgers, the clef, nothing else moves) · test_septet_notation 101 · the front-matter lines on NITS · NOTATION_STANDARDS §0b · FOUND for 2b: the print page draws no piano and no part labels (its first job)` —
  **Result when done:** the pitch form is a property of the REALIZATION. The presentation score (print + the jury's video) and the full ensemble rehearsal score show the bass clarinet at sounding pitch on a bass clef; the sectional scores, the individual rehearsal scores and any part show it in B♭ on a treble clef (sounding a major ninth lower — §382's form, the default and the working page's view). The IR stays sounding (D9); the shift is one registry override per realization, applied at layout (`writtenOf`); a test proves both forms; the front matter says which. **His words:** *"the whole transposing instrument mechanism … the standards and rules we need in place so that we can display a full score in C, i.e. the presentation score and the full ensemble rehearsal score. And then the other ones in the transposition."* **Standing mandate M5.**
  **The data (§541):** today `ensemble.json` part 1 `clef: treble · transpose: 14` everywhere — one number, one function; the trill's neighbour and the morph headers take the written pitch through the same path. Sounding range B♭1 … F4 (211 notes, 65 at or above C4, none above G4): on a bass clef at most two ledger lines above, no clef changes; the low B♭1 the first ledger below.
  1. **The decision, recorded** — D55 (done at the decision).
  2. **The registry** — the bass clarinet's `transpose` + `clef` per realization: the default (the page, sectionals, individual scores, parts) B♭ treble; a `realizations.<presentation>` (and the ensemble-rehearsal realization when it exists) override to `0 / bass`; the print and video exporters pass that ensemble to layout; the working page keeps the default (a toggle, if ever wanted, is a later nicety).
  3. **The C page proven** — ledgers ≤ 2 above the staff, B♭1 below; every trill neighbour and morph header re-spelled from the sounding note by the same function; nothing else on the page moves (item by item, the other six parts).
  4. **Guards** — `test_septet_notation`'s written-pitch rows both ways (the default B♭ treble; C bass under the presentation realization); the print exporter's proof on one page.
  5. **The front matter** — "score in C; the bass clarinet sounds as written" on the presentation score; "in B♭, sounding a major ninth lower" on the transposed versions (the NITS performance-instructions list).
  6. **Later versions** — the sectional, individual and part exporters take the default; the full ensemble rehearsal score takes the presentation override; **the performance score (PLAN 3) decides its own form at its design** (one shared display — the mechanism's hardest case).
  7. **The record** — NOTATION_STANDARDS §1 (the bass clarinet's row: the two forms) · RUNNING_LOG · commit + push.
...
- **2b — Presentation score** *(the title on the cover and in the format entry: **tentative _Scattered Substance_**, CN-64 — confirm with him before either is written)* — print PDF (A3 landscape, format entry + cover + performance
  **FOUND 2026-09-16 (PLAN 2k's print proof, RUNNING_LOG §552):** `export_print.js --htmlOnly --at 100` on piece-septet draws SIX systems — sys-p0 · p1 · p3 · p4 · p5 · p6 — and NO piano (the grand staff's systems are keyed 2:0 / 2:1; the static page keys the frame by part) and no part labels; the video exporter's frame still defaults to the tuba's ten parts. Both exporters now take the realized ensemble + techniques (2k), but their frame, lanes, labels and the grand staff are 2b's first job before any page is judged.
  ...
  **Top line:** 1 ⚠ the septet frame · 2 the paper (A3, marks) · 3 the proof pages — his eye · 4 the front matter · 5 the full render ·
  ...
  - **2b.7 — The page edges, by rule** — `BUILT 2026-09-17 (Opus, RUNNING_LOG §615): 7.1–7.6 in order. 63 → 68 pages; 14 532 of 14 532 point items owned exactly once; 1304 arcs = 1304 impact dots; 19 pages ragged, max 1.90 s (18 %); five checks green; the film byte-identical, proven twice. HIS EYE OWED, then 2b.6` — **Result when done:** on every
    ...
    **THE RULE (D59): a page OWNS [cut, next cut).** A POINT EVENT — a strike and everything attached to it — draws only on the page that owns its
    ...
    - 2b.7.2 ✓ (0.422 s) THE LEFT RESERVE: the window opens `max(musicStartBufferSs, GC.pre + margin)` before the cut, so the first owned strike's whole arc
      clears the gutter; the previous page's events inside that reserve are not drawn (ownership).
    - 2b.7.3 ✓ (0.302 s; 68 pages, as simulated) THE RIGHT RESERVE: print plans its pages at `pageSeconds − left reserve − (GC.post + margin)`, so the last owned strike's rebound and
      its marks are whole. Cost, simulated: **63 → 68 pages.**
    - 2b.7.4 ✓ (19 pages, max 1.90 s = 18 %) THE SYSTEM ENDS WHERE THE PAGE'S MUSIC ENDS — **HIS WORD: "a" (D59, §614), the ragged right edge:** staff lines, ruler and long items stop at the cut + the right
...
## 3. Performance score — `deferred` — port #4's modules when they exist there (D2). **Carries D42, the curve look (`docs/CURVE_LOOK.md`), in the rehearsal and performance scores — the composer, 2026-09-13.** **Carries D55 / M5, the transposing-instrument mechanism (PLAN 2k, 2026-09-16): every version declares its pitch form — the full ensemble rehearsal score in C; the sectional scores, the individual rehearsal scores and parts transposed (the bass clarinet in B♭ treble); the performance score the players read in concert decides its own form at its design (one shared display).** **Carries the MAIN IR's engraving overlays as written — D50's `dynFixed` (the morph section's piano dynamics, page-only: the save's velocities are NOT the written dynamic there), the beamed pairs, every other per-note override — the parts are drawn from the IR, never re-derived from the save. Acceptance: the performance score's piano marks in 205–428 s equal `tools/test_morph_notation.js`'s D50 table — the composer, 2026-09-14 (RUNNING_LOG §503).**
...
- **M5 — The pitch form is a property of the realization** (D55, composer 2026-09-16, RUNNING_LOG §541–§542): a full score read
  together — the presentation score, the full ensemble rehearsal score — is in C; a score read by one player — the sectional scores,
  the individual rehearsal scores, parts — is transposed (the bass clarinet in B♭ on a treble clef, sounding a major ninth lower). The IR
  is always sounding (D9); the shift is one registry override per realization applied at layout, never a second IR. **Every later score
  version declares its pitch form before it is built, and the mechanism (PLAN 2k) is in place before the first of them.** The performance
  score the players read in concert (PLAN 3, one shared display) decides its own form at its design.
